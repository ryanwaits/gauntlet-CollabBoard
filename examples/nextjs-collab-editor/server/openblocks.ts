...
const PORT = parseInt(process.env.OPENBLOCKS_PORT || "2002", 10);
...
const server = new OpenBlocksServer({
  ...
  initialYjs: async (roomId: string) => {
    ...
    if (existsSync(path)) {
      console.log(`[editor] Loading Yjs state for room ${roomId}`);
  ...
  onJoin: (roomId, user) => {
    console.log(`[editor] join: ${user.displayName} → room ${roomId}`);
  },
  onLeave: (roomId, user) => {
    console.log(`[editor] leave: ${user.displayName} ← room ${roomId}`);
  },
});

await server.start(PORT);
console.log(`[editor] Server listening on :${server.port}`);
